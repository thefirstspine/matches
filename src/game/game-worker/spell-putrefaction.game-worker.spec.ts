...
describe('Spell putrefaction', () => {
  ...
  /**
   * ======================================================================
   * TEST putrefaction SPELL GAME WORKER
   * ======================================================================
   */

  test('putrefaction on damaged card', async () => {
    // Get base cards
    const putrefactionCard = await restService.card('putrefaction');
    const bansheeCard = await restService.card('banshee');
    const towerCard = await restService.card('the-tower');
    expect(putrefactionCard).toBeDefined();
    ...
    const gameAction: IGameAction<any> =
      await gameWorkerService.getWorker('spell-putrefaction').create(gameInstance, {user: gameInstance.users[0].user});
    ...
    expect((gameAction.interaction as ISubActionPutCardOnBoard).params.boardCoords.length).toBe(1);
    ...
    // Execute
    const result: boolean = await gameWorkerService.getWorker('spell-putrefaction').execute(
      gameInstance,
      gameAction,
    );
    ...
    expect(gameInstance.cards[1].currentStats.life).toBe(bansheeCard.stats.life - 5);
    expect(gameInstance.cards[1].location).toBe('discard');
    expect(gameInstance.cards[0].location).toBe('discard');
  });

  test('putrefaction on non-damaged card', async () => {
    // Get base cards
    const putrefactionCard = await restService.card('putrefaction');
    const bansheeCard = await restService.card('banshee');
    const towerCard = await restService.card('the-tower');

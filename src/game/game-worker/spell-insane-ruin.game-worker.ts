...
/**
 * Game worker for "insane-ruin" spell.
 */
@Injectable() // Injectable required here for dependency injection
export class SpellInsaneRuinGameWorker implements IGameWorker, IHasGameHookService {

  public gameHookService: GameHookService;

  readonly type: string = 'spell-insane-ruin';
  ...
  public async create(gameInstance: IGameInstance, data: {user: number}): Promise<IGameAction<IInteractionPutCardOnBoard>> {
    return {
      ...
      type: this.type,
      name: {
        en: `Play Insane's Ruin`,
        fr: `Jouer une Ruine de Démence`,
      },
      description: {
        en: `Play Insane's Ruin on an artifact`,
        fr: `Jouer une Ruine de Démence sur un artefact`,
      },
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    this.arenaRoomsService.sendMessageForGame(
      gameInstance,
      {
        fr: `A joué une Ruine de Démence`,
        en: `Played Insane's Ruin`,
      },
      gameAction.user);

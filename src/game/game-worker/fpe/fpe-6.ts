...
export class Fpe6GameWorker implements IGameWorker, IHasGameHookService, IHasGameWorkerService {
  ...
  public async execute(gameInstance: IGameInstance, gameAction: IGameAction<IInteractionPutCardOnBoard>): Promise<boolean> {
    ...
    // Add next the action
    const action: IGameAction<any> = await this.gameWorkerService.getWorker('fpe-8').create(gameInstance, {user: gameAction.user});
